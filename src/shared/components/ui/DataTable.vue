  ...
  <div class="overflow-x-auto rounded border border-neutral-200">
    <table class="min-w-full divide-y divide-neutral-200">
      ...
      <tbody class="divide-y divide-neutral-200 bg-white">
        <tr v-if="loading">
          <td :colspan="columns.length" class="px-4 py-6 text-center text-neutral-500">Carregant…</td>
        </tr>
        <tr v-else-if="rows.length === 0">
          <td :colspan="columns.length" class="px-4 py-6 text-center text-neutral-500">Cap registre.</td>
        </tr>
        <tr v-else v-for="(row, idx) in rows" :key="idx" class="hover:bg-neutral-50">
